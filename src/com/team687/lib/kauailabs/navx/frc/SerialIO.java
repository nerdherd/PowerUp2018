...
class SerialIO implements IIOProvider {
	...
	is_usb = ((port_id == SerialPort.Port.kUSB) || (port_id == SerialPort.Port.kUSB1)
		...
		if (bytes_read > 0) {
			...
			if (packet_length > 0) {
				...
				packet_length = AHRSProtocol.decodeIntegrationControlResponse(received_data, i,
					bytes_remaining, integration_control_response);
				if (packet_length > 0) {
					...
					if (!next_packet_start_found) {
					    ...
					    if ((bytes_remaining > 2) && (received_data[i
    ...
    /**
     * Indicates whether the navX MXP is currently connected to the host computer. A
     * connection is considered established whenever a value update packet has been
     * received from the navX MXP within the last second.
     * 
    ...
    /**
     * Returns the count in bytes of data received from the navX MXP. This could can
    ...
    /**
     * Returns the count of valid update packets which have been received from the
     * navX MXP. This count should increase at the same rate indicated by the
	...
	enqueueIntegrationControlMessage((byte) (AHRSProtocol.NAVX_INTEGRATION_CTL_RESET_DISP_X
		| AHRSProtocol.NAVX_INTEGRATION_CTL_RESET_DISP_Y | AHRSProtocol.NAVX_INTEGRATION_CTL_RESET_DISP_Z));
    }
