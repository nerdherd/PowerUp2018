...
public class CenterToRightSwitchAuto extends CommandGroup {
	...
	addSequential(new ResetDriveEncoders());
	addSequential(new DriveStraightDistance(NerdyMath.inchesToTicks(AutoConstants.kRobotToSecondCubeSwitch), -45, 3, 0.5));
//	addSequential(new WaitTime(0.1));
	addSequential(new TurnTime(0.5, 0.5));
	addSequential(new TurnTime(-0.5, 0.5));
	addSequential(new ResetDriveEncoders());
	addSequential(new DriveStraightDistance(NerdyMath.inchesToTicks(-AutoConstants.kRobotToSecondCubeSwitch - 15), -135, 3, 0.5));
