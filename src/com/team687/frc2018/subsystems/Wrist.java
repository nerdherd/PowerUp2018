...
//	m_wrist.config_kF(0, 0, 0);
	...
	m_wrist.configMotionAcceleration(1500, 0);
    ...
    public double getDesiredAbsoluteAngleGoingDown() {
    	...
    	if (theta2 <= 55) {
    	    ...
    	    return alpha*45 + (1-alpha)*90 - _theta3_offset; // -1.75*[@theta2]+135.3-theta3_offset THIS HAS BEEN CHANGED
    	} else {
...
//	SmartDashboard.putNumber("Wrist Desired Position", angleAbsoluteToTicks(getDesiredAbsoluteAngle()));
...
//	SmartDashboard.putNumber("Wrist Roll", getRoll());
	...
	public void startLog() {
		...
		if(logFolder1.exists() && logFolder1.isDirectory())
			filePrefix = Paths.get(logFolder1.toString(), "2018_02_18_Wrist");
		else if(logFolder2.exists() && logFolder2.isDirectory())
			filePrefix = Paths.get(logFolder2.toString(), SmartDashboard.getString("log_file_name", "2018_02_18_Wrist"));
		...
		if(!writeException) {
			...
			try {
				...
				m_writer.append("Time,Position,DesiredPos,DesiredAngle,Velocity,PigeonAngle,EncoderAngle,Voltage,Current\n");
				m_writer.flush();
		...
		if (!writeException) {
			try {
				...
				m_writer.append(String.valueOf(timestamp) + "," + 
						String.valueOf(getPositionRelative()) + "," +
						String.valueOf(m_desiredPos) + "," + 
						String.valueOf(getDesiredAngle()) + "," +
