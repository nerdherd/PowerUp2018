...
//    	setPercentVelocity(leftPercent, rightPercent);
    	setPercentOutput(leftPercent, rightPercent);
    }
...
//    	m_rightMaster.set(ControlMode.PercentOutput, rightPercent);
	...
	public void startLog() {
		...
		if(logFolder1.exists() && logFolder1.isDirectory())
			...
			filePrefix = Paths.get(logFolder2.toString(), SmartDashboard.getString("log_file_name", "2018_02_10_Drive"));
		...
		if (!writeException) {
			try {
				...
				m_writer.append(String.valueOf(timestamp) + "," + 
						String.valueOf(getRightPosition()) + "," +
						String.valueOf(getLeftPosition()) + "," + 
						String.valueOf(getRightSpeed()) + "," + 
						String.valueOf(getLeftSpeed()) + "," + 
						...
						String.valueOf(Robot.pdp.getVoltage()) + "\n");
				m_writer.flush();
			} catch (IOException e) {
				e.printStackTrace();
				writeException = true;
